...
function MenuItem({
  ...
  return (
    ...
    >
      ...
      {shortcut && (
        <span className={getClassName("shortcut")}>{shortcut}</span>
      )}
    </button>
  );
}
 ...
 */
export function CanvasInteractions({ children }: { children: React.ReactNode }) {
  ...
  const portalRoot =
    typeof document !== "undefined" ? getPortalRoot() : null;
